...
本项目已将截图执行规范与清单固化在 `docs/系统测试文档.md` 第6节， 提交时按 `docs/images/system_test/` 目录归档。  

最少覆盖以下证据截图：

1. SQL 01~06 初始化成功截图（环境可复现证据）；
2. TC-01 下单锁车前后对比截图（规则A）；
3. TC-02 订单完成后车辆售出与时间戳截图（规则B）；
4. TC-04 非法下单报错与回滚核验截图（事务一致性）；
5. Q2 顾问业绩排行与 Q7 库存预警截图（统计能力）；
6. Python 三大菜单联调截图（应用功能完整性）。  
...
截图归档路径与命名规范见：`docs/images/system_test/README.md`。  
提交时按该清单补齐 PNG 证据文件， 并与第7.3.2节用例一一对应。
